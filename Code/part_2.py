...
def get_temp_flucs(csv):
    ...
    data_frame = pd.read_csv(csv, usecols=["station_id", "temperature_c"]).sort_values(
        by=["station_id"]
    )
    ...
    for item, row in data_frame.iterrows():
        ...
        # On first loop iteration there was no last row temperature so init last_row_temp to
        # current_row_temp, and there was no previous row station so set previous row station
        # id to current row station id
        if first_iteration:
        ...
        # Else if we are still on the same station then track fluctuations seen on this
        # station so far
            ...
            # cur_row_temp is updated in each loop so save it in prev_row_temp
            # since we are done tracking data
            prev_row_temp = cur_row_temp
        ...
        # if its not the first iteration and cur_row_station != row_station we must iterating
        # a new station's data so reset relevant vars. The stations temp data is sorted by
        # station_id so we can use a single iteration to find result
        else:
